  ...
  const ParticleSystemTest = () => {
    ...
    useFrame((state) => {
      ...
      particles.forEach((item, index) => {
        ...
        particles[index].aAngle =
          simplexA.noise3D(
            position[index * 3] * FREQUENCY,
            position[index * 3 + 1] * FREQUENCY,
            xyz / 100
          ) *
          AMPLITUDE *
          particles[index].speed;
        particles[index].bAngle =
          simplexB.noise3D(
            position[index * 3 + 1] * FREQUENCY,
            position[index * 3 + 2] * FREQUENCY,
            xyz / 100
          ) *
          AMPLITUDE *
          particles[index].speed;
        ...
        position[index * 3] +=
          Math.cos(particles[index].aAngle) *
          Math.cos(particles[index].bAngle) *
          STEP;
        position[index * 3 + 1] +=
          Math.sin(particles[index].aAngle) *
          Math.cos(particles[index].bAngle) *
          STEP;
        position[index * 3 + 2] += Math.sin(particles[index].bAngle) * STEP;

        let newC = new THREE.Vector3(
          Math.cos(particles[index].aAngle) * Math.cos(particles[index].bAngle),
        ...
        if (particles[index].newborn === true) {
          ...
          if (particles[index].decay < 20) size[index] -= 1;
        }
        /* if(Raytracing) {
        ...
        if (size[index] < 0) {
          ...
          particles[index].decay =
            (Math.random() * maxDecay) / 2 + maxDecay / 2;
        }
    ...
    return (
      <points
        /* onPointerOver={onPointerOver} */ ref={geom}
        position={[0, 0, 0]} /* rotation={[-Math.PI / 4, 0, Math.PI / 6]} */
      >
